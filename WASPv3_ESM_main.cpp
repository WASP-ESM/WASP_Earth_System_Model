...
//Coded by Philip Goodwin, This version 16-October-2020

//Citations for use of code:

//This version of the WASP model is an extension of:

//Goodwin, P. and B.B. Cael (2020, submitted) Bayesian estimation of Earth’s climate sensitivity and transient climate response from observational warming and heat content datasets, submitted to Earth System Dynamics Discussions

//Now with option of a palaeoclimate/geological based prior for long timescale climate sensitivity
...
//Main programme - include the following header files ...
#include <cmath>
#include <ctime>
#include <iostream>
#include <iomanip>
...
int main()
{
    ...
    int T_data =  2; //set T_data = 0 for HadCRUT4; T_data = 1 for Cowtan&Way v2.0.0; T_data = 2 for HadCRUT5 (&HadSST4); T_data = 3 for HadCRUT5 without infilling (&HadSST4) data constraints
    int OHC_data = 0; //set OHC_data = 0 for Cheng et al., and OHC_data = 1 for NODC  data constraints. NODC uses IPCC AR5 (2015) whole system heat content change since NODC only goes to 2000m.
    ...
    SCENARIOS = 10000000; //Perform this number of Monte Carlo simulations in the initial ensemble
    ...
    //print to screen which observational datasets are used in the filter
    if(T_data == 0)
        cout << "Observational filter uses HadCRUT4 surface temperatures ";
    if(T_data == 1)
        cout << "Observational filter uses Cowtan&Way v2.0 surface temperatures ";
    if(T_data == 2)
        cout << "Observational filter uses HadCRUT5 surface temperatures ";
    if(T_data == 3)
        cout << "Observational filter uses HadCRUT5 (no infilling) surface temperatures ";
    if(OHC_data == 0)
        cout << "and Cheng et al. ocean heat content" << endl;
    if(OHC_data == 1)
        cout << "and NODC ocean heat content" << endl;
    ...
    //Sets up the output files///////////////////////////////////////////////////////////////////
      ...
      ofstream results ("./RESULTS/Outputs_" + std::to_string(SEED) + ".txt", std::ios::out);
      
      //Outputs the model input values at 2016 in the observation-consistent ensemble members/////////////////////////////////////////////
      
      //Format:
      ofstream inputs ("./RESULTS/Inputs_" + std::to_string(SEED) + ".txt", std::ios::out);
      ...
      ofstream projection ("./RESULTS/Projection_" + std::to_string(SEED) + ".txt", std::ios::out);
      ...
      ofstream annual_S_effective ("./RESULTS/EffCS_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Effective lambda in Wm/2/K over time for all sources of radiative forcing using air temperature
      ofstream annual_lambda ("./RESULTS/lambda_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Instantaneous TCRE - using air-temperature (annual average?)
      ofstream Instantaneous_TCRE ("./RESULTS/Instantaneous_TCRE_" + std::to_string(SEED) + ".txt", std::ios::out);
      
      //Surface air temperature anomaly relative to 1850-1900 average///////
      ofstream annual_warming ("./RESULTS/Warming_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Surface blended temperature anomaly relative to 1850-1900 average///////
      ofstream annual_warming_blended ("./RESULTS/Warming_blended_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Sea Surface temperature anomaly relative to 1850-1900 average///////
      ofstream annual_warming_SST ("./RESULTS/Warming_SST_" + std::to_string(SEED) + ".txt", std::ios::out);
      
    //Annual heat imbalance from all sources
    ofstream annual_N_tot ("./RESULTS/N_ave_" + std::to_string(SEED) + ".txt", std::ios::out);
    
      //Radiative forcing from all sources
      ofstream annual_R_tot ("./RESULTS/R_tot_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Radiative forcing from all anthropogenic sources
      ofstream annual_R_anth ("./RESULTS/R_anth_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Radiative forcing from solar activity
      ofstream annual_R_solar ("./RESULTS/R_solar_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Radiative forcing from volcanic eruptions
      ofstream annual_R_volcanic("./RESULTS/R_volcanic_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Radiative forcing from CO2
      ofstream annual_RCO2 ("./RESULTS/RCO2_" + std::to_string(SEED) + ".txt", std::ios::out);
      //efficacy-weighted radiative forcing from aerosols and other non-Kyoto agents
      ofstream annual_R_aerosol ("./RESULTS/R_aerosol_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Radiative forcing from WMGHG other than CO2)
      ofstream annual_R_WMGHG_nonCO2 ("./RESULTS/R_WMGHG_nonCO2_" + std::to_string(SEED) + ".txt", std::ios::out);
      //R_CO2 divided by lambda_CO2 over time
      
      //Annual heat content anonaly of the upper 700m of the ocean
      ofstream annual_heat700m ("./RESULTS/OHC_700m_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Annual heat content anonaly of the whole ocean
      ofstream annual_heatOcean ("./RESULTS/OHC_Ocean_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Annual heat content anonaly of the Earth system
      ofstream annual_heatEarth ("./RESULTS/OHC_700_2000m_" + std::to_string(SEED) + ".txt", std::ios::out);
      
      //Residual terrestrial carbon storage anonaly relative to previous year
      ofstream annual_landC ("./RESULTS/LandC_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Residual terrestrial carbon storage anonaly relative to previous year
      ofstream annual_oceanC ("./RESULTS/OceanC_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Compatible cumulative emissions
      ofstream annual_Em ("./RESULTS/CumulativeEm_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Equivalent carbon emissions from ocean heating
      ofstream annual_I_eq_Heat ("./RESULTS/I_eq_heat_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Amount of carbon in the soil carbon pool
      ofstream landC_soil ("./RESULTS/SoilC_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Amount of carbon in the plant carbon pool
      ofstream landC_plant ("./RESULTS/PlantC_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Amount of carbon in the atmospheric CO2 pool (MtC)
      ofstream atmosCO2 ("./RESULTS/CO2_" + std::to_string(SEED) + ".txt", std::ios::out);
      //NPP carbon flux (MtC/yr)
      ofstream NPP_flux ("./RESULTS/NPP_flux_" + std::to_string(SEED) + ".txt", std::ios::out);
      //Airborne fraction of total emitted carbon
      ofstream Airborne_fraction ("./RESULTS/Airborne_fraction_" + std::to_string(SEED) + ".txt", std::ios::out);
    ...
    const int t_step_per_year = 12; //Number of timsteps per year
    ...
    */
    ...
    double gamma_Rnoise = 0.77;
    double alpha_Rnoise = 0.79;
    //Set noise settings according to number of timesteps per year:
    if(t_step_per_year == 48)
    {
        gamma_Rnoise = 0.77; //fractional relaxation in internal RF noise per timestep
        alpha_Rnoise = 0.79; //maximum growth of internal RF noise in W per m2 per timestep
    }
    if(t_step_per_year == 24)
    {
        gamma_Rnoise = 0.62; //fractional relaxation in internal RF noise per timestep
        alpha_Rnoise = 0.92; //maximum growth of internal RF noise in W per m2 per timestep
        
    }
    if(t_step_per_year == 12)
    {
        gamma_Rnoise = 0.45; //fractional relaxation in internal RF noise per timestep
        alpha_Rnoise = 0.96; //maximum growth of internal RF noise in W per m2 per timestep
    ...
    for (int j = 0; j<SCENARIOS; j++) //This loop runs 'SCENARIOS' number of ensemble members
    {
        ...
        if(RESTORE == 1 )
        {
            ...
            if(Future_scenario<15 && Use_stored_params == 0)
            {
                ...
                lambda_WVLR = getRandomLinear2(-3.0, 1.0, generator());//getRandomNormal2(lambda - lambda_Planck, (lambda-lambda_Planck)/1.0, generator() );//0.0;// getRandomNormal(-1.15,0.09); //getRandomLinear2(-3.15,2.0, generator());//         //Combined Water Vapour - Lapse Rate
                lambda_Cloud_Fast = 0.0;// Use 0.0 when doing just one getRandomNormal(-0.43,0.33);   //Clouds with original SST patterns
                lambda_Cloud_SST = getRandomLinear2(-3.0, 3.0, generator()); //getRandomNormal(0.0,3.3);//getRandomLinear2(-2.0, 2.0, generator()); //getRandomNormal(-0.47,0.30);    //Clouds with final SST patterns
                ...
                //lambda_WVLR = getRandomNormal2(lambda - lambda_Planck, (lambda-lambda_Planck)/1.0, generator() );//
                //lambda_Cloud_SST = lambda - lambda_Planck - lambda_WVLR;
        ...
        if(Use_stored_params == 0)
        {
            srand(j+1); //Seeds the random number generator for internal noise (and the random test variable) so that this can be repeated
        }
        if (Use_stored_params == 1)
        {
            srand(j_obs[j]+1);
        }
             ...
             
        {
           ...
           //DeltaT[i] = (1.0/lambda_time[2][i])*(1.0 - (epsilon*N_mixed[i])/(a_CO2*log(CO2[i]/CO2init) + epsilon_aero*R_aerosol[i] + R_WMGHG_nonCO2[i] + R_solar[i] + epsilon_volcanic*R_volcanic[i] ) ) * ( (a_CO2/Ib)* (I_em[i] + I_Usat_mixed[i] + I_Usat_deep[i] + I_Usat_upper[i] + I_Usat_inter[i] + I_Usat_bottom[i] + I_eq_heat[i]) + epsilon_aero*R_aerosol[i] + R_WMGHG_nonCO2[i] + R_solar[i] + R_volcanic[i] ) + T_noise[i];
            ...
            if ( year[i] >= 2020.0-dt/2.0 && year[i] <= 2020.0+dt/2.0  )
            {
                ...
                for (int n= (2000-Start_year)*t_step_per_year; n<(2020-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    DT_2000_2019 += DeltaT[n];
                }
                DT_2000_2019  =  DT_2000_2019/double((2020-2000)*t_step_per_year);
                
                //2000 to 2009 inclusive
                double DT_2000_2018 = 0.0;
                
                for (int n= (2000-Start_year)*t_step_per_year; n<(2019-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    DT_2000_2018 += DeltaT[n];
                }
                DT_2000_2018  =  DT_2000_2018/double((2019-2000)*t_step_per_year);
                ...
                DT_2000_2018 = DT_2000_2018 - DT_1961_1990;*/
                
                DT_1990_1999 = DT_1990_1999 - DT_2000_2018;
                DT_1980_1999 = DT_1980_1999 - DT_2000_2018;
                DT_1960_1979 = DT_1960_1979 - DT_2000_2018;
                DT_1940_1959 = DT_1940_1959 - DT_2000_2018;
                DT_1920_1939 = DT_1920_1939 - DT_2000_2018;
                DT_1900_1919 = DT_1900_1919 - DT_2000_2018;
                DT_1850_1899 = DT_1850_1899 - DT_2000_2018;
                ...
                //Ocean heat content for NODC /////////////////////////////////
                double Ocean_Heat_0_700m_1955_2006 = 0.0;
                
                for (int n= (1955-Start_year)*t_step_per_year; n<(2007-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_700m_1955_2006 += Heat_mixed[n] + Heat_upper[n];
                }
                Ocean_Heat_0_700m_1955_2006  =  Ocean_Heat_0_700m_1955_2006/double((2007-1955)*t_step_per_year);
                
                double Ocean_Heat_0_2000m_1955_2006 = 0.0;
                
                for (int n= (1955-Start_year)*t_step_per_year; n<(2007-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_2000m_1955_2006 += Heat_mixed[n] + Heat_upper[n] +Heat_inter[n] + (0.15/0.21)*Heat_deep[n];
                }
                Ocean_Heat_0_2000m_1955_2006  =  Ocean_Heat_0_2000m_1955_2006/double((2007-1955)*t_step_per_year);
                
                
                double Ocean_Heat_0_700m_2015_2019 = 0.0;
                
                for (int n= (2015-Start_year)*t_step_per_year; n<(2020-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_700m_2015_2019 += Heat_mixed[n] + Heat_upper[n];
                }
                Ocean_Heat_0_700m_2015_2019  =  Ocean_Heat_0_700m_2015_2019/double((2020-2015)*t_step_per_year);
                
                double Ocean_Heat_0_2000m_2015_2019 = 0.0;
                
                for (int n= (2015-Start_year)*t_step_per_year; n<(2020-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_2000m_2015_2019 += Heat_mixed[n] + Heat_upper[n] +Heat_inter[n] + (0.15/0.21)*Heat_deep[n];
                }
                Ocean_Heat_0_2000m_2015_2019  =  Ocean_Heat_0_2000m_2015_2019/double((2020-2015)*t_step_per_year);
                
                double Ocean_Heat_0_700m_1955_1959 = 0.0;
                
                for (int n= (1955-Start_year)*t_step_per_year; n<(1960-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_700m_1955_1959 += Heat_mixed[n] + Heat_upper[n];
                }
                Ocean_Heat_0_700m_1955_1959  =  Ocean_Heat_0_700m_1955_1959/double((1960-1955)*t_step_per_year);
                
                double Ocean_Heat_0_2000m_1955_1959 = 0.0;
                
                for (int n= (1955-Start_year)*t_step_per_year; n<(1960-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_2000m_1955_1959 += Heat_mixed[n] + Heat_upper[n] +Heat_inter[n] + (0.15/0.21)*Heat_deep[n];
                }
                Ocean_Heat_0_2000m_1955_1959  =  Ocean_Heat_0_2000m_1955_1959/double((1960-1955)*t_step_per_year);
                
                //Re-set to NODC reference period
                Ocean_Heat_0_2000m_2015_2019 = Ocean_Heat_0_2000m_2015_2019 - Ocean_Heat_0_2000m_1955_1959;
                Ocean_Heat_0_700m_2015_2019 = Ocean_Heat_0_700m_2015_2019 - Ocean_Heat_0_700m_1955_1959;
                
                
                
                double Total_ES_HC_1971_2010 = ((Heat_mixed[(2011-Start_year)*t_step_per_year-1] + Heat_upper[(2011-Start_year)*t_step_per_year-1] + Heat_inter[(2011-Start_year)*t_step_per_year-1] + Heat_deep[(2011-Start_year)*t_step_per_year-1] + Heat_bottom[(2011-Start_year)*t_step_per_year-1])
                -  (Heat_mixed[(1971-Start_year)*t_step_per_year] + Heat_upper[(1971-Start_year)*t_step_per_year] + Heat_inter[(1971-Start_year)*t_step_per_year] + Heat_deep[(1971-Start_year)*t_step_per_year] + Heat_bottom[(1971-Start_year)*t_step_per_year]))/f_heat_ocean ;
                ...
                //The time slots for Zanna et al OHC dataset, 0 to 700m
                double Ocean_Heat_0_700m_2000_2018 = 0.0;
                
                for (int n= (2000-Start_year)*t_step_per_year; n<(2019-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_700m_2000_2018 += Heat_mixed[n] + Heat_upper[n];
                }
                Ocean_Heat_0_700m_2000_2018  =  Ocean_Heat_0_700m_2000_2018/double((2019-2000)*t_step_per_year);
                
                double Ocean_Heat_0_700m_1980_1999 = 0.0;
                for (int n= (1980-Start_year)*t_step_per_year; n<(2000-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_700m_1980_1999 += Heat_mixed[n] + Heat_upper[n];
                }
                Ocean_Heat_0_700m_1980_1999  =  Ocean_Heat_0_700m_1980_1999/double((2000-1980)*t_step_per_year);
                
                double Ocean_Heat_0_700m_1960_1979 = 0.0;
                for (int n= (1960-Start_year)*t_step_per_year; n<(1980-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_700m_1960_1979 += Heat_mixed[n] + Heat_upper[n];
                }
                Ocean_Heat_0_700m_1960_1979  =  Ocean_Heat_0_700m_1960_1979/double((1980-1960)*t_step_per_year);
                
                double Ocean_Heat_0_700m_1940_1959 = 0.0;
                for (int n= (1940-Start_year)*t_step_per_year; n<(1960-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_700m_1940_1959 += Heat_mixed[n] + Heat_upper[n];
                }
                Ocean_Heat_0_700m_1940_1959  =  Ocean_Heat_0_700m_1940_1959/double((1960-1940)*t_step_per_year);
                
                double Ocean_Heat_0_700m_1920_1939 = 0.0;
                for (int n= (1920-Start_year)*t_step_per_year; n<(1940-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_700m_1920_1939 += Heat_mixed[n] + Heat_upper[n];
                }
                Ocean_Heat_0_700m_1920_1939  =  Ocean_Heat_0_700m_1920_1939/double((1940-1920)*t_step_per_year);
                
                double Ocean_Heat_0_700m_1900_1919 = 0.0;
                for (int n= (1900-Start_year)*t_step_per_year; n<(1920-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_700m_1900_1919 += Heat_mixed[n] + Heat_upper[n];
                }
                Ocean_Heat_0_700m_1900_1919  =  Ocean_Heat_0_700m_1900_1919/double((1920-1900)*t_step_per_year);
                
                double Ocean_Heat_0_700m_1870_1899 = 0.0;
                for (int n= (1870-Start_year)*t_step_per_year; n<(1900-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_700m_1870_1899 += Heat_mixed[n] + Heat_upper[n];
                }
                Ocean_Heat_0_700m_1870_1899  =  Ocean_Heat_0_700m_1870_1899/double((1900-1870)*t_step_per_year);
                
                //The time slots for Zanna et al OHC dataset, 0 to 200m
                double Ocean_Heat_0_2000m_2000_2018 = 0.0;
                
                for (int n= (2000-Start_year)*t_step_per_year; n<(2019-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_2000m_2000_2018 += Heat_mixed[n] + Heat_upper[n] +Heat_inter[n] + (0.15/0.21)*Heat_deep[n];
                }
                Ocean_Heat_0_2000m_2000_2018  =  Ocean_Heat_0_2000m_2000_2018/double((2019-2000)*t_step_per_year);
                
                double Ocean_Heat_0_2000m_1980_1999 = 0.0;
                for (int n= (1980-Start_year)*t_step_per_year; n<(2000-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_2000m_1980_1999 += Heat_mixed[n] + Heat_upper[n] +Heat_inter[n] + (0.15/0.21)*Heat_deep[n];
                }
                Ocean_Heat_0_2000m_1980_1999  =  Ocean_Heat_0_2000m_1980_1999/double((2000-1980)*t_step_per_year);
                
                double Ocean_Heat_0_2000m_1960_1979 = 0.0;
                for (int n= (1960-Start_year)*t_step_per_year; n<(1980-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_2000m_1960_1979 += Heat_mixed[n] + Heat_upper[n] +Heat_inter[n] + (0.15/0.21)*Heat_deep[n];
                }
                Ocean_Heat_0_2000m_1960_1979  =  Ocean_Heat_0_2000m_1960_1979/double((1980-1960)*t_step_per_year);
                
                double Ocean_Heat_0_2000m_1940_1959 = 0.0;
                for (int n= (1940-Start_year)*t_step_per_year; n<(1960-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_2000m_1940_1959 += Heat_mixed[n] + Heat_upper[n] +Heat_inter[n] + (0.15/0.21)*Heat_deep[n];
                }
                Ocean_Heat_0_2000m_1940_1959  =  Ocean_Heat_0_2000m_1940_1959/double((1960-1940)*t_step_per_year);
                
                double Ocean_Heat_0_2000m_1920_1939 = 0.0;
                for (int n= (1920-Start_year)*t_step_per_year; n<(1940-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_2000m_1920_1939 += Heat_mixed[n] + Heat_upper[n] +Heat_inter[n] + (0.15/0.21)*Heat_deep[n];
                }
                Ocean_Heat_0_2000m_1920_1939  =  Ocean_Heat_0_2000m_1920_1939/double((1940-1920)*t_step_per_year);
                
                double Ocean_Heat_0_2000m_1900_1919 = 0.0;
                for (int n= (1900-Start_year)*t_step_per_year; n<(1920-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_2000m_1900_1919 += Heat_mixed[n] + Heat_upper[n] +Heat_inter[n] + (0.15/0.21)*Heat_deep[n];
                }
                Ocean_Heat_0_2000m_1900_1919  =  Ocean_Heat_0_2000m_1900_1919/double((1920-1900)*t_step_per_year);
                
                double Ocean_Heat_0_2000m_1870_1899 = 0.0;
                for (int n= (1870-Start_year)*t_step_per_year; n<(1900-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_0_2000m_1870_1899 += Heat_mixed[n] + Heat_upper[n] +Heat_inter[n] + (0.15/0.21)*Heat_deep[n];
                }
                Ocean_Heat_0_2000m_1870_1899  =  Ocean_Heat_0_2000m_1870_1899/double((1900-1870)*t_step_per_year);
                
                //The time slots for Zanna et al OHC dataset, whole ocean
                double Ocean_Heat_whole_2000_2018 = 0.0;
                
                for (int n= (2000-Start_year)*t_step_per_year; n<(2019-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_whole_2000_2018 += Heat_mixed[n] + Heat_upper[n] + Heat_inter[n] + Heat_deep[n] + Heat_bottom[n];
                }
                Ocean_Heat_whole_2000_2018  =  Ocean_Heat_whole_2000_2018/double((2019-2000)*t_step_per_year);
                
                double Ocean_Heat_whole_1980_1999 = 0.0;
                for (int n= (1980-Start_year)*t_step_per_year; n<(2000-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_whole_1980_1999 += Heat_mixed[n] + Heat_upper[n] + Heat_inter[n] + Heat_deep[n] + Heat_bottom[n];
                }
                Ocean_Heat_whole_1980_1999  =  Ocean_Heat_whole_1980_1999/double((2000-1980)*t_step_per_year);
                
                double Ocean_Heat_whole_1960_1979 = 0.0;
                for (int n= (1960-Start_year)*t_step_per_year; n<(1980-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_whole_1960_1979 += Heat_mixed[n] + Heat_upper[n] + Heat_inter[n] + Heat_deep[n] + Heat_bottom[n];
                }
                Ocean_Heat_whole_1960_1979  =  Ocean_Heat_whole_1960_1979/double((1980-1960)*t_step_per_year);
                
                double Ocean_Heat_whole_1940_1959 = 0.0;
                for (int n= (1940-Start_year)*t_step_per_year; n<(1960-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_whole_1940_1959 += Heat_mixed[n] + Heat_upper[n] + Heat_inter[n] + Heat_deep[n] + Heat_bottom[n];
                }
                Ocean_Heat_whole_1940_1959  =  Ocean_Heat_whole_1940_1959/double((1960-1940)*t_step_per_year);
                
                double Ocean_Heat_whole_1920_1939 = 0.0;
                for (int n= (1920-Start_year)*t_step_per_year; n<(1940-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_whole_1920_1939 += Heat_mixed[n] + Heat_upper[n] + Heat_inter[n] + Heat_deep[n] + Heat_bottom[n];
                }
                Ocean_Heat_whole_1920_1939  =  Ocean_Heat_whole_1920_1939/double((1940-1920)*t_step_per_year);
                
                double Ocean_Heat_whole_1900_1919 = 0.0;
                for (int n= (1900-Start_year)*t_step_per_year; n<(1920-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_whole_1900_1919 += Heat_mixed[n] + Heat_upper[n] + Heat_inter[n] + Heat_deep[n] + Heat_bottom[n];
                }
                Ocean_Heat_whole_1900_1919  =  Ocean_Heat_whole_1900_1919/double((1920-1900)*t_step_per_year);
                
                double Ocean_Heat_whole_1870_1899 = 0.0;
                for (int n= (1870-Start_year)*t_step_per_year; n<(1900-Start_year)*t_step_per_year; n++)//Add up contributions
                {
                    Ocean_Heat_whole_1870_1899 += Heat_mixed[n] + Heat_upper[n] + Heat_inter[n] + Heat_deep[n] + Heat_bottom[n];
                }
                Ocean_Heat_whole_1870_1899  =  Ocean_Heat_whole_1870_1899/double((1900-1870)*t_step_per_year);
                
                
                Ocean_Heat_whole_1870_1899 = Ocean_Heat_whole_2000_2018 - Ocean_Heat_whole_1870_1899;
                Ocean_Heat_whole_1900_1919 = Ocean_Heat_whole_2000_2018 - Ocean_Heat_whole_1900_1919;
                Ocean_Heat_whole_1920_1939 = Ocean_Heat_whole_2000_2018 - Ocean_Heat_whole_1920_1939;
                Ocean_Heat_whole_1940_1959 = Ocean_Heat_whole_2000_2018 - Ocean_Heat_whole_1940_1959;
                Ocean_Heat_whole_1960_1979 = Ocean_Heat_whole_2000_2018 - Ocean_Heat_whole_1960_1979;
                Ocean_Heat_whole_1980_1999 = Ocean_Heat_whole_2000_2018 - Ocean_Heat_whole_1980_1999;
                
                Ocean_Heat_0_2000m_1870_1899 = Ocean_Heat_0_2000m_2000_2018 - Ocean_Heat_0_2000m_1870_1899;
                Ocean_Heat_0_2000m_1900_1919 = Ocean_Heat_0_2000m_2000_2018 - Ocean_Heat_0_2000m_1900_1919;
                Ocean_Heat_0_2000m_1920_1939 = Ocean_Heat_0_2000m_2000_2018 - Ocean_Heat_0_2000m_1920_1939;
                Ocean_Heat_0_2000m_1940_1959 = Ocean_Heat_0_2000m_2000_2018 - Ocean_Heat_0_2000m_1940_1959;
                Ocean_Heat_0_2000m_1960_1979 = Ocean_Heat_0_2000m_2000_2018 - Ocean_Heat_0_2000m_1960_1979;
                Ocean_Heat_0_2000m_1980_1999 = Ocean_Heat_0_2000m_2000_2018 - Ocean_Heat_0_2000m_1980_1999;
                
                Ocean_Heat_0_700m_1870_1899 = Ocean_Heat_0_700m_2000_2018 - Ocean_Heat_whole_1870_1899;
                Ocean_Heat_0_700m_1900_1919 = Ocean_Heat_0_700m_2000_2018 - Ocean_Heat_0_700m_1900_1919;
                Ocean_Heat_0_700m_1920_1939 = Ocean_Heat_0_700m_2000_2018 - Ocean_Heat_0_700m_1920_1939;
                Ocean_Heat_0_700m_1940_1959 = Ocean_Heat_0_700m_2000_2018 - Ocean_Heat_0_700m_1940_1959;
                Ocean_Heat_0_700m_1960_1979 = Ocean_Heat_0_700m_2000_2018 - Ocean_Heat_0_700m_1960_1979;
                Ocean_Heat_0_700m_1980_1999 = Ocean_Heat_0_700m_2000_2018 - Ocean_Heat_0_700m_1980_1999;
                ...
                if (T_data == 1)
                {
                    //Global Mean Surfae Temperature, Cowtan and Way Had4krig annual v2.0.0
                ...
                if (T_data == 2)
                {
                    //Global Mean Surfae Temperature, HadCRUT5
                    cost_function = cost_function * exp( -0.5*(((DT_2000_2018 - 0.614)*(DT_2000_2018 - 0.614))/(0.016*0.016)));
                    cost_function = cost_function * exp( -0.5*(((DT_1980_1999 - 0.240)*(DT_1980_1999 - 0.240))/(0.017*0.017)));
                    cost_function = cost_function * exp( -0.5*(((DT_1960_1979 - (-0.092))*(DT_1960_1979 - (-0.092)))/(0.022*0.022)));
                    cost_function = cost_function * exp( -0.5*(((DT_1940_1959 - (-0.052))*(DT_1940_1959 - (-0.052)))/(0.062*0.062)));
                    cost_function = cost_function * exp( -0.5*(((DT_1920_1939 - (-0.206))*(DT_1920_1939 - (-0.206)))/(0.059*0.059)));
                    cost_function = cost_function * exp( -0.5*(((DT_1900_1919 - (-0.428))*(DT_1900_1919 - (-0.428)))/(0.066*0.066)));
                    cost_function = cost_function * exp( -0.5*(((DT_1850_1899 - (-0.359))*(DT_1850_1899 - (-0.359)))/(0.072*0.072)));
                ...
                if (T_data == 2)
                {
                    ...
                    cost_function = cost_function * exp( -0.5*(((DT_1980_1999 - (-0.374))*(DT_1980_1999 - (-0.374)))/(0.023*0.023)));
                    cost_function = cost_function * exp( -0.5*(((DT_1960_1979 - (-0.706))*(DT_1960_1979 - (-0.706)))/(0.027*0.027)));
                    cost_function = cost_function * exp( -0.5*(((DT_1940_1959 - (-0.666))*(DT_1940_1959 - (-0.666)))/(0.064*0.064)));
                    cost_function = cost_function * exp( -0.5*(((DT_1920_1939 - (-0.820))*(DT_1920_1939 - (-0.820)))/(0.061*0.061)));
                    cost_function = cost_function * exp( -0.5*(((DT_1900_1919 - (-1.042))*(DT_1900_1919 - (-1.042)))/(0.067*0.067)));
                    cost_function = cost_function * exp( -0.5*(((DT_1850_1899 - (-0.973))*(DT_1850_1899 - (-0.973)))/(0.074*0.074)));
                ...
                {
                    //Global Mean Surfae Temperature, HadCRUT5
                    cost_function = cost_function * exp( -0.5*(((DT_2000_2018 - 0.589)*(DT_2000_2018 - 0.589))/(0.033*0.033)));
                    cost_function = cost_function * exp( -0.5*(((DT_1980_1999 - 0.246)*(DT_1980_1999 - 0.246))/(0.034*0.034)));
                    cost_function = cost_function * exp( -0.5*(((DT_1960_1979 - (-0.089))*(DT_1960_1979 - (-0.089)))/(0.036*0.036)));
                    cost_function = cost_function * exp( -0.5*(((DT_1940_1959 - (-0.064))*(DT_1940_1959 - (-0.064)))/(0.069*0.069)));
                    cost_function = cost_function * exp( -0.5*(((DT_1920_1939 - (-0.203))*(DT_1920_1939 - (-0.203)))/(0.065*0.065)));
                    cost_function = cost_function * exp( -0.5*(((DT_1900_1919 - (-0.423))*(DT_1900_1919 - (-0.423)))/(0.074*0.074)));
                    cost_function = cost_function * exp( -0.5*(((DT_1850_1899 - (-0.315))*(DT_1850_1899 - (-0.315)))/(0.090*0.090)));
                    
                }*/
                
                if (T_data == 3)
                {
                    //Global Mean Surfae Temperature, HadCRUT5 (no infilling)
                    
                    cost_function = cost_function * exp( -0.5*(((DT_1980_1999 - (-0.343))*(DT_1980_1999 - (-0.343)))/(0.048*0.048)));
                    cost_function = cost_function * exp( -0.5*(((DT_1960_1979 - (-0.678))*(DT_1960_1979 - (-0.678)))/(0.049*0.049)));
                    cost_function = cost_function * exp( -0.5*(((DT_1940_1959 - (-0.653))*(DT_1940_1959 - (-0.653)))/(0.076*0.076)));
                    cost_function = cost_function * exp( -0.5*(((DT_1920_1939 - (-0.792))*(DT_1920_1939 - (-0.792)))/(0.073*0.073)));
                    cost_function = cost_function * exp( -0.5*(((DT_1900_1919 - (-1.012))*(DT_1900_1919 - (-1.012)))/(0.081*0.081)));
                    cost_function = cost_function * exp( -0.5*(((DT_1850_1899 - (-0.904))*(DT_1850_1899 - (-0.904)))/(0.096*0.096)));
                    
                }
                
                //Global Mean SST
                if(T_data < 2) //HadSST3 for HadCRUT4 and prior
                    cost_function = cost_function*exp( -0.5*(((SST_1850_1899 - (-0.276))*(SST_1850_1899 - (-0.276)))/(0.076*0.076)));
                
                if(T_data >= 2)//HadSST4 for HadCRUT5 and after
                    cost_function = cost_function*exp( -0.5*(((SST_1850_1899 - (-0.281))*(SST_1850_1899 - (-0.281)))/(0.105*0.105)));
                    
                
                
                //Choose OHC Cheng et al or combination of NODC ocean layers and IPCC Whole system
                ...
                /*
                ////////////////////////////////////////////////
                //This is Zanna et al. 2019
                //cost_function = cost_function*exp( -0.5*(((Ocean_Heat_whole_1870_1899 - 3.897e23)*(Ocean_Heat_whole_1870_1899 - 3.897e23))/(5.05e22*5.05e22)));
                //cost_function = cost_function*exp( -0.5*(((Ocean_Heat_whole_1900_1919 - 4.245e23)*(Ocean_Heat_whole_1900_1919 - 4.245e23))/(5.04e22*5.04e22)));
                //cost_function = cost_function*exp( -0.5*(((Ocean_Heat_whole_1920_1939 - 4.092e23)*(Ocean_Heat_whole_1920_1939 - 4.092e23))/(3.89e22*3.89e22)));
                cost_function = cost_function*exp( -0.5*(((Ocean_Heat_whole_1940_1959 - 2.923e23)*(Ocean_Heat_whole_1940_1959 - 2.923e23))/(3.77e22*3.77e22)));
                //cost_function = cost_function*exp( -0.5*(((Ocean_Heat_whole_1960_1979 - 2.293e23)*(Ocean_Heat_whole_1960_1979 - 2.293e23))/(3.22e22*3.22e22)));
                //cost_function = cost_function*exp( -0.5*(((Ocean_Heat_whole_1980_1999 - 1.322e23)*(Ocean_Heat_whole_1980_1999 - 1.322e23))/(2.55e22*2.55e22)));
                
                //cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_2000m_1870_1899 - 3.733e23)*(Ocean_Heat_0_2000m_1870_1899 - 3.733e23))/(5.05e22*5.05e22)));
                //cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_2000m_1900_1919 - 4.092e23)*(Ocean_Heat_0_2000m_1900_1919 - 4.092e23))/(5.03e22*5.03e22)));
                //cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_2000m_1920_1939 - 3.931e23)*(Ocean_Heat_0_2000m_1920_1939 - 3.931e23))/(3.88e22*3.88e22)));
                cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_2000m_1940_1959 - 2.799e23)*(Ocean_Heat_0_2000m_1940_1959 - 2.799e23))/(3.76e22*3.76e22)));
                cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_2000m_1960_1979 - 2.206e23)*(Ocean_Heat_0_2000m_1960_1979 - 2.206e23))/(3.22e22*3.22e22)));
                cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_2000m_1980_1999 - 1.241e23)*(Ocean_Heat_0_2000m_1980_1999 - 1.241e23))/(2.55e22*2.55e22)));
                
                //cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_700m_1870_1899 - 2.93e23)*(Ocean_Heat_0_700m_1870_1899 - 2.93e23))/(3.99e22*3.99e22)));
                //cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_700m_1900_1919 - 3.271e23)*(Ocean_Heat_0_700m_1900_1919 - 3.271e23))/(3.97e22*3.97e22)));
                //cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_700m_1920_1939 - 3.038e23)*(Ocean_Heat_0_700m_1920_1939 - 3.038e23))/(3.0e22*3.0e22)));
                cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_700m_1940_1959 - 2.01e23)*(Ocean_Heat_0_700m_1940_1959 - 2.01e23))/(3.02e22*3.02e22)));
                cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_700m_1960_1979 - 1.665e23)*(Ocean_Heat_0_700m_1960_1979 - 1.665e23))/(2.5e22*2.5e22)));
                cost_function = cost_function*exp( -0.5*(((Ocean_Heat_0_700m_1980_1999 - 8.98e22)*(Ocean_Heat_0_700m_1980_1999 - 8.98e22))/(1.99e22*1.99e22)));
                /////////////////////////////////////////////////////////
                */
                //Ocean carbon uptake
                ...
                if(cost_function > cost_function_max)
                {
                    ...
                    cout << j << '\t' << cost_function_max << '\t' << lambda << '\t' << lambda_WVLR << '\t' << lambda_Cloud_SST <<  endl;
                }
                //Check against random number whose maximum is linked to which observation-constraints are used
                if(T_data == 0 && OHC_data == 0)//HadCRUT4 + Cheng et al.
                    random_test = getRandomLinear(0.0,0.35);
                if(T_data == 1 && OHC_data == 0)//Cowtan&Way + Cheng et al.
                    random_test = getRandomLinear(0.0,0.025);
                if(T_data == 0 && OHC_data == 1)//HadCRUT4 + NODC
                    random_test = getRandomLinear(0.0,0.35);
                if(T_data == 1 && OHC_data == 1)//Cowtan&Way + NODC
                    random_test = getRandomLinear(0.0,0.025);
                if(T_data == 2 && OHC_data == 0)//HadCRUT5 + Cheng et al.
                    random_test = getRandomLinear(0.0,0.18);
                if(T_data == 3 && OHC_data == 0)//HadCRUT5 (no infilling) + Cheng et al.
                    random_test = getRandomLinear(0.0,0.18);
                
                
                //Accept the simulation if the random number is larger than the cost function
               ...
               */
               ...
               //If use sea level rise component, add in additional sea level observational constraints here
                ...
                if(data_fit >= 10)
                {
                    ...
                    cout << "Simulation" << '\t' <<  j << '\t' << "is observation consistent" << '\t' << cost_function  << '\t' << lambda << '\t' << lambda_WVLR << '\t' << lambda_Cloud_SST << endl;
                ...
                if(data_fit>=10)
                {
                    ...
                    //Output stored properties by year
                        for (int i2 = -years_back; i2 <=0; i2 ++)
                        {
                            ...
                            double Rtot_ave_11yr = 0.0;
                            double N_ave_11_yr = 0.0;
                            double T_ave_11_yr = 0.0;
                            
                            //Calculate 11-year averages
                            if(i2 <= - 6 && i2 >= - years_back + 5)
                            {
                                
                                for(int n=0; n<11*t_step_per_year; n++ )
                                {
                                    T_ave_11_yr += DeltaT[i+(i2+5)*t_step_per_year - n]/double(11*t_step_per_year);
                                    N_ave_11_yr += N_mixed[i+(i2+5)*t_step_per_year - n]/double(11*t_step_per_year);
                                    Rtot_ave_11yr += (a_CO2*log(CO2[i+(i2+5)*t_step_per_year - n]/CO2init) + R_aerosol[i+(i2+5)*t_step_per_year - n] + R_WMGHG_nonCO2[i+(i2+5)*t_step_per_year - n] + R_volcanic[i+(i2+5)*t_step_per_year - n] + R_solar[i+(i2+5)*t_step_per_year - n])/double(11*t_step_per_year);
                                    
                                }
                                annual_lambda << (Rtot_ave_11yr - N_ave_11_yr)/(T_ave_11_yr) << '\t' ;
                                
                                annual_S_effective << (T_ave_11_yr*a_CO2*log(2) ) / (Rtot_ave_11yr - N_ave_11_yr) << '\t' ;
                                
                            }
                            else//return zeros if outside of 11-year average range (i.e. end and start of simulation)
                            {
                                annual_lambda << 0.0 << '\t';
                                annual_S_effective << 0.0 << '\t';
                            }
    ...
    if(a_CO2_obs.size()>=1)
    {
        stored_inputs << std::setprecision(std::numeric_limits<double>::max_digits10) ;
        ...
        for (int i = 0; i<count_obs-1; i++)
        {
            stored_inputs << j_obs[i] << "," ;
        }
        stored_inputs << j_obs[count_obs-1] << "};" << endl;
